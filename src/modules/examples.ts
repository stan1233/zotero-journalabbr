...
import {
  ...
  FeildExport, // 字段导出
} from "../abbrtools/baseselect";
import { journal_abbr as journal_abbr_nlm } from "../abbrtools/data";
import { journal_abbr as journal_abbr_legacy } from "../abbrtools/legacyData";
import { replaceHandle, filterValidEntries } from "../abbrtools/replacehandle";
...
export class BasicExampleFactory {
  ...
  static async initPrefs() {
    const initpref_data = {
      ...
      [config.addonRef + ".autorunabbrauto"]: false,
  ...
  static registerPrefs() {
    Zotero.PreferencePanes.register({
      ...
      label: getString("prefs-abbr-title"),
...
export class UIExampleFactory {
  ...
  static registerRightClickMenuItem() {
    ...
    ztoolkit.Menu.register("item", {
      ...
      label: getString("menupopup-abbr-export"), //期刊缩写导出
  ...
  static registerRightClickMenuPopup() {
    ...
    ztoolkit.Menu.register("item", {
      ...
      label: getString("menupopup-abbr-label"), //期刊缩写
      children: [
        {
          tag: "menuitem",
        ...
        },
        {
          ...
          commandListener: (ev) =>
            HelperAbbrFactory.JA_removeTagname([
              "abbr",
              "abbr_nlm",
              "abbr_legacy",
            ]),
        ...
        {
          tag: "menuitem",
          id: "zotero-itemmenu-abbr-journal-abbrmanual",
          label: "abbrmanual",
          commandListener: (ev) =>
            HelperAbbrFactory.JA_transferItemsToCustomField(true),
        },
        {
          tag: "menuitem",
  ...
  // 主面板添加一个额外的列 --- 但是这个不能正确的显示标题
  // @example
  // static async registerExtraColumn() {
  //   const field = "columnAbbr";// 主面板添加一个额外的列
  //   await Zotero.ItemTreeManager.registerColumns({
  //     pluginID: addon.data.config.addonID,
  //     dataKey: field,
  //     label: "abbr",
  //     dataProvider: (item: Zotero.Item, dataKey: string) => {
  //       return ztoolkit.ExtraField.getExtraField(item, "itemBoxRowabbr") || "";
  ...
  static async registerExtraColumnWithCustomCell() {
    const field = "columnAbbr2"; // 主面板添加一个额外的列-能够显示标题
    await Zotero.ItemTreeManager.registerColumns({
      pluginID: addon.data.config.addonID,
      dataKey: field,
      label: "abbr",
  ...
  static registerItemPaneCustomInfoRow() {
    Zotero.ItemPaneManager.registerInfoRow({
      rowID: "itemBoxRowabbr", // 右面板添加行
      pluginID: addon.data.config.addonID,
      editable: true,
      label: {
        l10nID: getLocaleID("item-info-row-abbr-label"),
      },
      ...
      onSetData: ({ item, value }) => {
        ztoolkit.ExtraField.setExtraField(item, "itemBoxRowabbr", value);
...
export class HelperAbbrFactory {
  ...
  static async JA_removeTagname(
    ...
    const tagLabel = usertags.join(", ");
    await Basefun.processSelectedItemsWithPromise(
      SelectedWithHandler.removeTagHandler(usertags),
      getString("prompt-success-removetag-info") + ": " + tagLabel,
      getString("prompt-error-removetag-info") + ": " + tagLabel,
      true,
      selectedItems,
    );
  }
  ...
  static JA_processBuiltinTags(isselect_addAutotags: boolean) {
    const clearTags = [
      "abbr",
      "abbr_nlm",
      "abbr_legacy",
    ...
    return {
      clearTags,
      sourceTags: isselect_addAutotags
        ? {
            nlm: ["abbr_nlm"],
            legacy: ["abbr_legacy"],
          }
  ...
  /**
   * 1.对选中的条目进行处理, 优先采用 NLM 数据集对期刊缩写进行更新.
   * 如果没有命中, 则回退到冻结的 legacy 数据集.
   * @param {Array<Zotero.Item>} [selectedItems] 要处理的项目数组（可选）
   */
  static async JA_update_UseInnerData(selectedItems?: Array<Zotero.Item>) {
    ...
    const { sourceTags, clearTags } =
      this.JA_processBuiltinTags(isselect_addAutotags);

    await Selected.updateJournalAbbrWithFallback(
      journal_abbr_nlm,
      journal_abbr_legacy,
      "publicationTitle",
      "journalAbbreviation",
  ...
  /**
   *主要是在菜单栏中进行点击操作, 点击的标签是: abbrauto,  对选中的文献, 采用一键更新期刊缩写, 然后对选中的文献(不同类别的文献)期刊字段, 转移到自定义字段 `itemBoxRowabbr` 中(在面板中显示的是 abbr 值)
   * @param {boolean} [isshowinfo=true] 是否显示提示信息
   * @param {Array<Zotero.Item>} [selectedItems] 要处理的项目数组（可选）
   * @returns {Promise} 返回一个 Promise 对象
   */
  ...
  static async JA_transferItemsToCustomField(
    isshowinfo: boolean = true,
    selectedItems?: Array<Zotero.Item>,
  ): Promise<any> {
    // 方法二: 直接根据不同的文献类型,把不同类型的字段转移到自定义字段 `itemBoxRowabbr` 中(在右侧面板中显示的是 abbr 值)
    // 如果这些规定的字段中没有值, 则会替换为默认值
    try {
      await Selected.transferAllItemsToCustomField(selectedItems);
    } catch (error) {
      ztoolkit.log(`journalabbr error: ${error}`);
